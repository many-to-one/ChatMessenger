...
class CustomTokenAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if request.method == 'POST' and 'login' in request.path:
            ...
            # access_token = f'{uuid4()}{uuid4()}{uuid4()}'
        ...
        if request.method == 'POST' and 'accessToken' in request.path:
            ...
            if auth and auth[0].lower() == b'bearer':
                ...
                print('ACCESS AUTHENTICATION -----------------------------', access_token)
                ...
                user = get_object_or_403(CustomUser, access_token=access_token)

                # Check if the token has been added to the blacklist (dead_token)
                dead_token = BlackListToken.objects.filter(token=access_token)
                if dead_token:
                    print('ACCESS dead_token -----------------------------', dead_token)
                ...
                if user:
                    ...
                    if time > timedelta(minutes=20):
                        print('ACCESS timedelta > -------------------------')
                        ...
                        user.save()
                        print('ACCESS return (user, token) -------------------------')
